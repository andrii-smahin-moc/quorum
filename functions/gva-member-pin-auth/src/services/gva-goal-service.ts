...
//         return this.buildHandlerResultPayload({
//           responseId: this.config.gvaGoals.initiateOtpFlow,
//           customJourneyContext,
//         });
//       }
...
import { GliaAuthApi, QuorumApi } from '../apis';
import { GliaTransferApi } from '../apis/glia-transfer-api';
import { INITIAL_STEP, MEMBER_NUMBER_REGEX, MEMBER_PIN_REGEX, ZERO_NUMBER, OTP_IDENTIFIER_REGEX, OTP_CODE_REGEX } from '../constants';
import { GliaKVValueSchema } from '../schemas';
import { FunctionConfig, HandlerPayload, HandlerResult, IdentifierFailedAttemptsHistory, KvStoreFactory, LoggerInterface } from '../types';
import { validateSchema } from '../validator';
...
export class GVAGoalService extends BaseGVAGoalService {
  ...
  constructor(
    ...
    this.answerDetectorService = new AnswerDetectorService(this.config, this.logger, [
      ...
      new AnswerOption(AnswerOptionsList.ZERO_NUMBER, [ZERO_NUMBER, 'zero']),
      // NEW (OTP)
      new AnswerOption(AnswerOptionsList.OTP_IDENTIFIER, [OTP_IDENTIFIER_REGEX]),
  ...
  async validateMemberNumber(context: HandlerPayload): Promise<HandlerResult> {
    ...
    if (detectedAnswer && detectedAnswer.name === AnswerOptionsList.MEMBER_NUMBER && detectedAnswer.matchedText) {
      ...
      // NEW (OTP flow start after 503 "user not enrolled")
      if (isMemberExistsResponse && isMemberExistsResponse.statusCode === 503) {
        customJourneyContext.STEP = GVAGoalSteps.VALIDATE_OTP_IDENTIFIER;
        // init OTP attempts
        customJourneyContext.otpAttemptNumber = 0;
        customJourneyContext.otpAttemptLimit = this.config.inputValidationFailedAttemptsLimit;
        // persist base key for OTP attempts (per engagement)
        customJourneyContext.otpAttemptsKey = `OTP:${context.engagementId}`;

        return this.buildHandlerResultPayload({
          responseId: this.config.gvaGoals.otpflowstart, // GOAL_OTP_FLOW_START
          customJourneyContext,
          responseData: {
            // from .dev.vars (e.g. "Social security number")
            identifierType: (this.config as any).validateOtpIdentifier ?? 'Social security number',
          },
        });
      }
  ...
  async validatePin(context: HandlerPayload): Promise<HandlerResult> {
    ...
    const customJourneyContextPinned = this.getCustomJourneyContext(context);
    customJourneyContextPinned.STEP = GVAGoalSteps.VALIDATE_PIN;

    return this.buildHandlerResultPayload({
      customJourneyContext: customJourneyContextPinned,
      responseData: {
        pinAttemptLimit: this.config.inputValidationFailedAttemptsLimit,
        pinAttemptNumber: newFailedAttempts.length,
      },
  ...
  private async validateOtpIdentifier(context: HandlerPayload): Promise<HandlerResult> {
    ...
    const customJourneyContext = this.getCustomJourneyContext(context);
    const identifierType: string = (this.config as any).validateOtpIdentifier ?? 'Social security number';
    ...
    // не 9-значний → картка "Your answer is wrong, enter valid {{identifierType}}"
    customJourneyContext.STEP = GVAGoalSteps.VALIDATE_OTP_IDENTIFIER;
    return this.buildHandlerResultPayload({
      customJourneyContext,
  ...
  private async validateOtpCode(context: HandlerPayload): Promise<HandlerResult> {
    ...
    const customJourneyContext = this.getCustomJourneyContext(context);
    const otpAttemptsKey: string =
      (typeof customJourneyContext.otpAttemptsKey === 'string' && customJourneyContext.otpAttemptsKey) || `OTP:${context.engagementId}`;

    const attemptLimit = this.config.inputValidationFailedAttemptsLimit;
    const prevAttempts = await this.getFailedAttempts(otpAttemptsKey);

    // якщо валідний 6-значний код — успіх (як домовлялись)
    if (detectedAnswer && detectedAnswer.name === AnswerOptionsList.OTP_CODE && detectedAnswer.matchedText) {
      await this.resetFailedAttemptsHistory(otpAttemptsKey);

      // Тут міг би бути реальний бек-чек OTP, але за твоєю постановкою просто показуємо success-картку
      return this.buildHandlerResultPayload({
        isFinalStep: true,
        responseId: this.config.gvaGoals.successfullyVerifiesMemberNumberAndPin,
      });
    ...
    // інакше — невалідний OTP → інкрементуємо спроби і показуємо invalidotp
    const newAttempts = await this.saveFailedAttempt(otpAttemptsKey, prevAttempts);
